    ...
    while 1:
      ...
      if not self.__mainStack.isEmpty():
        ...
        while 1:
          ...
          if self.__abort:
            self.sig_log.emit(logSeverity.warning.value, "grblComSerial : L175, aborting while waiting for Grbl response...")
            self.sig_log.emit(logSeverity.warning.value, "grblComSerial : dernier Gcode envoyé = [{}]".format(lastSent))
        ...
        while 1:
          ...
          QCoreApplication.processEvents()
          if self.__abort:
            self.sig_log.emit(logSeverity.warning.value, "grblComSerial : L236, aborting while waiting for Grbl response...")
            break # Sortie de la boucle de lecture
